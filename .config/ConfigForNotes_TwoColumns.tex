% 若编译失败，且生成 .synctex(busy) 辅助文件，可能下面几种原因：

% - PDF 文件在外部被打开 (例如 Adobe Acrobat), 将其关闭即可
% - \includegraphics{} 需要插入的图片不存在：Ctrl + F 将这些代码注释/删除掉即可
% - 路径/文件名含中文或空格：更改路径/文件名即可
...
% 导入基本宏包
    ...
    \usepackage{multicol}   % 宏包：支持多栏 
        \setlength{\columnsep}{0.25in}    % 设置两栏之间的间距
    \usepackage{makecell}  % 宏包：表格单元格内换行
    ...
    \usepackage{etoolbox} % 用于调整某些环境的布局、间距等
        %\AtBeginEnvironment{graybox}{\vspace*{-5mm}}
        \AtBeginEnvironment{multicols*}{\vspace*{-5mm}} % 调整 chapter 标题与内容的间距
        %\AtEndEnvironment{multicols*}{\vspace*{-50mm}}
    %\usepackage{gensymb}  % 埃米符号 \angstrom 等
    %\usepackage{textcomp}
    %\newcommand{\myangstrom}{\mbox{\normalfont\AA}}
    \usepackage{siunitx}
    \usepackage{fontawesome}    % 宏包：更多符号, 例如对号错号
...
\usepackage{framed}     % 实现方框效果
    ...
    \definecolor{graybox_color}{rgb}{0.95, 0.95, 0.95} % 这里的 rbg 范围是 [0, 1]
...
\graphicspath{{\subfix{../}}}   % 修改主文件图像路径，使得子文件能够直接使用相对路径，而不是从 assets 开始索引
