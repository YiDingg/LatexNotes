...
\begin{figure}[H]\centering
\includegraphics[width=0.5\textwidth]{assets/1,2/7d3f6db847882c6447cc046186ede6ef.jpg}
\includegraphics[width=0.5\textwidth]{assets/1,2/3f5d73ec5ae9909ab42cd864852b0d72.jpg}
...
\begin{figure}[H]\centering
\includesvg[width=0.55\textwidth]{assets/1,2/信号分类.drawio.svg}
...
\begin{figure}[H]\centering
\includegraphics[width=0.43\textwidth]{assets/1,2/平衡电桥.drawio.pdf}
...
\begin{figure}[H]\centering
\includegraphics[width=0.7\textwidth]{assets/1,2/YDelta等效变换.drawio.pdf}
...
\begin{figure}[H]\centering
\includegraphics[width=0.5\textwidth]{assets/1,2/独立源等效.drawio.pdf}
...
\begin{figure}[H]\centering
\includegraphics[width=0.4\textwidth]{assets/1,2/独立电流源并联.drawio.pdf}
...
可以发现，减法器是在加法器的基础上，改变了 $u_+$ 端的电压而得。同理，向 $u_-$ 端并联电压源，或往 $u_+$ 端串联电压源，可得到能作任意项加减法的加减法器。但不能得到同相加法器，因为这样会存在输入电阻为 0 的端口，实际不可行。
\begin{enumerate}
...
\begin{figure}[H]\centering
    \begin{subfigure}[t]{0.4\textwidth}\centering
        \includegraphics[height=100pt]{assets/1,2/同相比例放大器.png}
        \caption{\bfseries 同相比例放大器}
    \end{subfigure}\begin{subfigure}[t]{0.4\textwidth}\centering
        \includegraphics[height=100pt]{assets/1,2/反向比例放大器.png}
...
\begin{subfigure}[t]{0.42\textwidth}\centering
    \includegraphics[height=100pt]{assets/1,2/反相加法器.png}
    \caption{\bfseries 反相加法器}
\end{subfigure}\begin{subfigure}[t]{0.4\textwidth}\centering
    \includegraphics[height=100pt]{assets/1,2/减法器.png}
...
\begin{subfigure}[t]{0.43\textwidth}\centering
    \includegraphics[height=130pt]{assets/1,2/压控电流源.png}
    \caption{\bfseries 压控电流源 VCCS}
\end{subfigure}\begin{subfigure}[t]{0.43\textwidth}\centering
    \includegraphics[height=130pt]{assets/1,2/负电阻.png}
...
\item \href{如何记住二端口ZYTH参数? - 雨过天晴的回答 - 知乎
https://www.zhihu.com/question/357342024/answer/1477595759}{如何记住二端口ZYTH参数?}
\item 
\end{enumerate}

\subsection{二端口的概念和参数}


\begin{definition}[二端口网络]
...
大多数时候，我们仅关系二端口的 $u$-$i$ 关系，其参数定义如下：
...
\begin{tabular}{cccccccccc}\toprule
    因变量& 自变量 & 参数矩阵\\
    \midrule
    $i_1, i_2$ & $u_1, u_2$ & 电导矩阵 $\boldsymbol{G}$ (conductance parameter)\\
    $u_1, u_2$ & $i_1, i_2$ & 电阻矩阵 $\boldsymbol{R}$ (resistance parameter)\\
    $u_1, i_1$& $u_2, i_2$&  传输矩阵 $\boldsymbol{T}$ (transmission parameter) \\
    $u_1, i_2$& $u_2, i_1$&  混合矩阵 $\boldsymbol{H}$ (hybrid parameter matrix)\\
    \bottomrule
\end{tabular}
\end{table}
