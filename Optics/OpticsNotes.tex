...
\begin{gather}
\begin{aligned}
    &\text{折射角：} && \cos \theta_t = \sqrt{1 - \left( \frac{\eta_i}{\eta_t} \sin \theta_i\right)^2} \\ 
    &\text{s 波强度反射率：}   && R_s = \left| \frac{\eta_i \cos \theta_i - \eta_t \cos \theta_t}{\eta_i \cos \theta_i + \eta_t \cos \theta_t} \right|^2 \\ 
    &\text{p 波强度反射率：}   && R_p = \left| \frac{\eta_i \cos \theta_t - \eta_t \cos \theta_i}{\eta_i \cos \theta_t + \eta_t \cos \theta_i} \right|^2 \\ 
    & \text{总强度反射率：} && F(\theta_i) = \frac{1}{2}\left( R_s + R_p \right)
\end{aligned}
\end{gather}
...
\begin{tabular}{cccccccccc}\toprule
    波类型 & 振幅系数 $r$ & 强度反射率 $R$\\
    \midrule
    s 波& $\displaystyle r_p = \frac{n_i\cos \theta_i - n_t \cos \theta_t}{n_i\cos \theta_i - n_t \cos \theta_t}$& $R_s = | r_s |^2$ \\
    p 波& $\displaystyle r_p = \frac{n_t\cos \theta_i - n_i \cos \theta_t}{n_t\cos \theta_i - n_i \cos \theta_t}$& $R_p = | r_p |^2$ \\
    符号3& 含义3& 单位3\\
    \bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]\centering
    ...
    \begin{tabular}{|c|c|c|}\hline
        类型 & 振幅反射系数 $r$ &  振幅透射系数 $t$ \\
        \hline
        s 波& $\displaystyle r_p = \frac{n_i\cos \theta_i - n_t \cos \theta_t}{n_i\cos \theta_i + n_t \cos \theta_t} = - \frac{\sin (\theta_i - \theta_t) }{\sin (\theta_i + \theta_t)}$ & $\displaystyle t_s  = \frac{2n_i \cos \theta_i}{n_i\cos \theta_i + n_t \cos \theta_t} = + \frac{2 \sin \theta_t \cos \theta_i}{\sin (\theta_i + \theta_t)}$ \\ 
        \hline
        p 波& $\displaystyle r_p = \frac{n_t\cos \theta_i - n_i \cos \theta_t}{n_t\cos \theta_i + n_i \cos \theta_t} = + \frac{\tan (\theta_i - \theta_t)}{\tan (\theta_i + \theta_t)} $ & $\displaystyle t_p  = \frac{2n_i \cos \theta_i}{n_i\cos \theta_t + n_t \cos \theta_i} = + \frac{2 \sin \theta_t \cos \theta_i}{\sin (\theta_i + \theta_t) \cos (\theta_i - \theta_t)}$  \\
        \hline
    \end{tabular}
\end{table}
...
强度反射率为：
\begin{equation}
    R_s = r_s^2,\ R_p = r_p^2, \quad  F(\theta_i) = \frac{1}{2}\left( R_s + R_p \right)
\end{equation}

如果 $1 - \left( \frac{\eta_i}{\eta_t} \sin \theta_i\right)^2 < 0$，则发生全反射，此时 $F(\theta_i) = 1$。

另外，需要指出菲涅尔公式的适用条件，也即推导时所做的一些假设，如下：
\begin{enumerate}
...
由菲涅尔公式，我们有如下结论：
\begin{gather}
r_p|_{\theta_i = 0} = (-r_s)|_{\theta_i = 0} = \frac{n_t - n_i}{n_t + n_i} \\ 
r_p|_{\theta_i = \frac{\pi}{2}} = r_s|_{\theta_i = \frac{\pi}{2}} = -1
\end{gather}
这表明，即使是正射（垂直于介质分界面的入射），一般也存在反射情况。并且，对不同的入射角 $\theta_i$
\begin{LineTheorem}[半波损失]\label{半波损失}

    


当
https://zhuanlan.zhihu.com/p/607510257
\end{LineTheorem}
