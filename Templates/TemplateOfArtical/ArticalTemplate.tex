...
% definition 环境自定义
    \newtheoremstyle{MySubsubsubsectionStyle}% <name>
        {11pt}% <space above>
        {11pt}% <space below>
        {}% <body font> 使用默认正文字体
        {}% <indent amount>
        ...
        {\textbf{#3}}% 设置标题内容顺序
    \theoremstyle{MySubsubsubsectionStyle} % 应用自定义的定理样式
...
\usepackage{titlesec}   
    % section标题自定义设置 
    \titleformat{\section}[hang]{\normalfont\huge\bfseries\centering}{\,\thesection\,}{20pt}{}
    % subsection标题自定义设置 
...
\end{center}
% \vspace{-0.2cm}
\noindent\rule{\textwidth}{0.1em}\zihao{-4}   % 分割线

\pagenumbering{Roman} % 页码为大写罗马数字
\setcounter{tocdepth}{2}  % 设置目录的深度为2，即只显示section和subsection  
\tableofcontents    % 目录页  
...
\section*{\huge\centering\bfseries 附录 A}\addcontentsline{toc}{section}{附录 A}\setcounter{subsection}{0}\renewcommand\thesubsection{A.\arabic{subsection}}
