...
$\forall\  z \in C$，$u(x,y) = x^2$ 和 $v(x,y) = xy$ 在 $\C$ 上有连续一阶偏导，因此 $f$ 在 $\C$ 上解析（解析必可导）。
...
$\forall\  z \in C$，$u(x,y) = (x-y)^2$ 和 $v(x,y) = 2(x+y)$ 在 $\C$ 上有连续一阶偏导，因此 $f$ 在 $\C$ 上解析（解析必可导）。
...
\end{equation*}
不在点 $(0,1)$ 上解析是因为在某点解析是指在此点的有心领域上解析，显然这里不满足。
...
偏导要满足 C-R 条件，代入得到：
\begin{gather*}
x^2 = y^2,\  2xy = 0, \quad \forall\  \sqrt{x^2+y^2} < 1  \\
...
\Longrightarrow f(x,y)= (x^2 + y^2 + x )+ i(2xy + y) + C 
\end{gather*}
...
\Longrightarrow f(x,y) = (e^y\cos x) + i(-e^y\sin x ) + C 
\end{gather*}
\end{enumerate}
...
\Longrightarrow v(x,y) = -x^3 + 3xy^2 + C_1, \quad u(x,y) = 3x^2y - y^3 + C_2 \\ 
\Longrightarrow f(x,y) = (3x^2y - y^3) + i(-x^3 + 3xy^2) + C
\end{gather*}
...
\begin{enumerate}
\item $u(x,y) = e^x(x\cos y - y\sin y) + 2\sin x \cdot \sin hy + x^3 - 3xy^2 +y$
