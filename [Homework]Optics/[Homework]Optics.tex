...
\section{对圆盘衍射，当圆盘恰好包含 $n$ 个半波带时( $n$ 个半波带被遮挡)，为何中心为亮斑？}
\section{波长为 634.8 nm 的平行光射向直径为 2.76 mm 的圆孔，与孔相距 1 m 处放一屏。试问：(1) 屏上正对圆孔中心的 $P$ 点是亮点还是暗点？
光栅的自由光谱范围？ (2) 要使 $P$ 点变成与 (1) 相反的情况，至少要把屏幕分别向前或向后移动多少？}
\section{一波带片由五个环带组成，第一环带是半径 $0 \sim r_1$ 的不透明圆盘，第二环带
半径 $r_1\sim r_2$ 透明，第三环带半径 $r_2\sim r_3$ 不透明，第四环带半径 $r_3\sim r_4$ 透明，第五环带是 $r_4 \sim \infty$ 的不透明区域。用波长 500 nm 的平行单色光照明，最亮的像点在距波带片 1 m 的轴上，试求：(1) $r_1$；(2) 像点的光强；(3) 其它光强极大值出现在轴上的哪些位置。}
\section{一東波长为 500 nm 的平行光垂直照射在一个单缝上。(2) 如果所用的单缝的宽度 $a=0.5$ mm，缝后紧挨着的薄透镜焦距 $f=1$ m ，试求： (1) 中央明条纹的角宽度； (2) 中央亮纹的线宽
度; (3) 第一级与第二级暗纹的距离。}
