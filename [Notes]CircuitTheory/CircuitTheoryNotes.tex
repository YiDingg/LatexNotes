...
\begin{cases}
    0 & u_{\text{GS}} < U_T \\
    \frac{K}{2}(u_{\text{GS}} - U_T)^2\cdot \frac{u_{\text{DS}}}{(u_{\text{DS}} - U_T)} & u_{\text{GS}} \geq U_T\ \& \ u_{\text{DS}} < u_{\text{GS}} - U_T \\ 
...
为了传递这些数字信号，最常见的方法是（n 条线）并行传输，也可以串行传输（比较少）。
