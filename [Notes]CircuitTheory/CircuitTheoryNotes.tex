...
一个{\color{red} 不含独立源 }的二端口网络可以用一个简单的二端口来等效，满足所需的参数矩阵相同即可。具体我们不谈，详见教材。
...
MOSFET standing for Metal Oxide Semiconductor Field Effect Transistor，它是数字电路的核心元素。小功率 FET 有 JFET、MOSFET，大功率 FET 有 VDMOS、IGBT 等，本课程主要面向 MOSFET。
...
%\begin{figure}[H]\centering
%    \includesvg[width=0.35\textwidth]{assets/1,2/MOS管符号.drawio.svg}
%    \caption{\textbf{N-MOS 示意图}}\label{N-MOS 示意图}
%\end{figure}

举个例子，$U_T = 1$ V，$K = 1\ \mathrm{mA \cdot V^{-2}}$ 时，MOS 管的电流特性如图 \ref{MOS 管的 iDS 电流特性} 所示\footnote{Matlab 源码见附录 \ref{MOS 管的 iDS 电流特性源码}}。
...
\end{subfigure}
\caption{\bfseries MOS 管的 $i_{\text{DS}}$ 电流特性 }\label{MOS 管的 iDS 电流特性}
\end{figure}
...
\begin{figure}[H]\centering
\includegraphics[width=0.5\textwidth]{assets/2/MOS管等效电路.png}
