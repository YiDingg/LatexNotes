...
\section{一阶动态电路应用}
此部分详见参考文献 \cite{电路原理} Page 207-223，我们不多赘述。


\subsection{MOSFET 的传播延迟 (NOT Gate)}
\subsection{RC 微分积分电路}
\subsection{OPA 微分积分电路}
\subsection{脉冲序列下的 RC 电路}
\subsection{降压斩波器 (DC > DC)}
\subsection{滤波全桥整流 (AC > DC)}

\chapter{动态电路的稳态分析}\thispagestyle{fancy}
