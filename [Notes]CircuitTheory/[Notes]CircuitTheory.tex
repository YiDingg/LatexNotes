...
例如，$U_T = 1$ V，$K = 1\ \mathrm{mA \cdot V^{-2}}$ 时，N 沟道增强型 MOSFET 的电流特性如图 \ref{MOS 管的 iDS 电流特性}\footnote{Matlab 源码见附录 \ref{MOS 管的 iDS 电流特性源码}} 所示。
...
注意，不能直接用叠加定理求解功率（功率为 2 次方，非线性），应先求出电压或电流，得到叠加后的结果，再计算功率。
\subsection{齐性定理}
...
% 附录 B
\chapter*{附录 B\hspace*{20pt} 电路原理 Q \& A}\setcounter{chapter}{21}   % 20-A, 21-B, 用于避免超链接跳转冲突
