...
\begin{BlockTheorem}[特勒根定理，Tellegen's Theorem\footnote{又称为特勒根定理似功率守恒定理 (Tellegen's Quasi-power Theorem)}]\label{特勒根定理}
    ...
    \end{equation}
\end{BlockTheorem}
特勒根定理是由基尔霍夫定律直接导出的，因此适用于任意电路。特别地，作为常见的情况，对{\color{red} 仅含电阻的二端口}（不要求电阻线性）有：

\noindent
\begin{equation}
\sum u_k \cdot \hat{i}_k = \sum \hat{u}_k \cdot i_k 
\end{equation}

\section{互易定理}

\begin{BlockTheorem}[互易定理，Reciprocal Theorem]\label{互易定理}
给定一个 {\color{red} 仅含有线性电阻}的二端口。互易定理的第一种形式表明，依次在二端口两端接入电压源激励 $u_{s1}$ 和 $u_{s2}$，未接入激励的端口短路，依次有信号电流 $i_1$ 和 $i_2$，则前后有关系：
\begin{equation}
    u_{s1}i_2 = u_{s2}i_1
\end{equation}
互易定理的第二种形式表明，依次在二端口两端接入电流源激励 $i_{s1}$ 和 $i_{s2}$，未接入激励的端口开路，依次有信号电压 $u_1$ 和 $u_2$，则前后有关系：
\begin{equation}
    u_1i_{s2} = u_2i_{s1}
\end{equation}
\end{BlockTheorem}

互易定理是仅含线性电阻的电路 $\boldsymbol{R}$ 参数与 $\boldsymbol{G}$ 参数互易的结果（这要求 $\boldsymbol{R} = \boldsymbol{R}^T$，$\boldsymbol{G} = \boldsymbol{G}^T$）。特别地，对于常用的情况，交换激励位置而大小不变，则得到的信号也交换位置而大小不变。
...
\begin{table}[H]\centering
    %\renewcommand{\arraystretch}{1.5} % 调整行间距为 1.5 倍
    %\setlength{\tabcolsep}{1.5mm} % 调整列间距
    \caption{\textbf{定理适用范围总结}}
    \label{定理适用范围总结}
\begin{tabular}{cccccccccc}\toprule
    定理 & 范围  \\
    \midrule
    节点电压法、回路电流法 &线性电阻电路（线性电阻、线性受控源、独立电源） \\
    替代定理 & 任意电路（可非线性、非时变） \\
    叠加定理、齐性定理 & 线性电路（线性元件、独立电源） \\
    戴维南定理、诺顿定理 & 线性电阻电路（线性电阻、线性受控源、独立电源）\\
    特勒根定理 & 任意电路（可非线性、非时变） \\
    互易定理 & 线性电路（线性元件、独立电源） \\
    对偶原理 &  \\
    \bottomrule
